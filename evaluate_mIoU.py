...
def get_args():
    ...
    parser.add_argument(
        "--gts", default="/ssd_scratch/cvit/girish.varma/dataset/anue_test/gtFine/test"
    )
...
def main(args):
    ...
    for i, gtf in enumerate(gts_folders):
        ...
        # print(g)
        p = [
            j.replace(gtf, pred_folders[i]) for j in g
        ]  # .replace('_gtFine_labellevel3Ids.png','_leftImg8bit.png')
        gts += g
        preds += p
